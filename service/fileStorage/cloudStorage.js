...
class CloudStorage {
    ...
    async save() {
        const { public_id: publicId, secure_url: avatarUrl } =
            await this.uploadCloud(this.filePath, {
                public_id: this.idAvatarCloud,
                folder: this.folderAvatars,
            });

        const newPublicId = publicId.replace(`${this.folderAvatars}/`, '');

        await Users.updateAvatar(this.userId, avatarUrl, newPublicId);
